...
class GameMonitoringService:
    ...
    def get_current_game_state_with_server_format(self, room_id=None, room_name=None, log_always=True, desired_pb_count=15):
        ...
        Args:
            ...
            desired_pb_count (int): 원하는 결과 개수
        ...
        try:
            ...
            if game_state:
                ...
                if log_always:
                    ...
                    self.logger.info(f"✅ iframe 게임 상태 분석 완료: 라운드 {round_num}, 결과 {result_count}개")
            else:
    ...
    def _parse_game_results_from_iframe(self, desired_count=15):
        """
        iframe에서 직접 게임 결과 파싱
        
        Args:
            desired_count (int): 원하는 결과 개수
            
        Returns:
            dict: 게임 상태 정보
        """
        try:
            self.logger.info(f"🔍 iframe에서 게임 결과 직접 파싱 시작 (최대 {desired_count}개)")
            ...
            if game_results or latest_result or round_number:
                ...
                self.logger.info(f"📊 iframe 파싱 결과: 라운드={round_number}, 결과={len(game_results or [])}개, 최신={latest_result}")
                return game_state
            else:
                self.logger.warning("❌ iframe에서 게임 데이터를 찾을 수 없습니다")
                return None
                
        except Exception as e:
            self.logger.error(f"iframe 게임 결과 파싱 오류: {e}")
            return None
    ...
    def _find_game_results(self, desired_count=None):
        """iframe에서 Bead Road의 SVG 좌표를 이용한 게임 결과 목록 찾기"""
        try:
            self.logger.info(f"🎯 Bead Road 게임 결과 파싱 시작" + (f" (최대 {desired_count}개)" if desired_count else " (제한 없음)"))
            ...
            # desired_count가 지정된 경우에만 제한 적용 (호환성 유지)
            if desired_count is not None and len(sorted_results) > desired_count:
                sorted_results = sorted_results[-desired_count:]
                self.logger.info(f"🔍 최근 {desired_count}개 결과: {sorted_results}")
            else:
                if desired_count is not None:
                    self.logger.info(f"🔍 결과 개수가 {desired_count}개 이하이므로 모든 결과 사용")
                else:
                    self.logger.info(f"🔍 제한 없이 모든 결과 사용: {len(sorted_results)}개")
            return sorted_results
                
        except Exception as e:
            self.logger.error(f"SVG 게임 결과 찾기 오류: {e}")
            return []
            ...
            for selector in latest_selectors:
                try:
                    ...
                    for element in elements:
                        ...
                        if result:
                            self.logger.debug(f"최신 결과 발견: {result}")
                            return result
                except:
                    continue
            
            # 2. 일반적인 결과에서 마지막 것 사용
            results = self._find_game_results(5)  # 최근 5개만
            if results:
                latest = results[-1]
                self.logger.debug(f"결과 목록에서 최신 결과: {latest}")
                return latest
            
            self.logger.warning("최신 게임 결과를 찾을 수 없습니다")
            return ''
            
        except Exception as e:
            self.logger.error(f"최신 결과 찾기 오류: {e}")
            return ''
    ...
    def _extract_pb_from_element(self, element):
        ...
        try:
            ...
            self.logger.debug(f"요소에서 PB 추출 오류: {e}")
            return []
    ...
    def _extract_pb_from_text(self, text, max_count=15):
        ...
        try:
            ...
            self.logger.debug(f"텍스트에서 PB 추출 오류: {e}")
            return []
    ...
    def _log_in_websocket_hybrid_format(self, game_state, room_id=None, room_name=None):
        ...
        try:
            ...
            self.logger.info(f"📊 최근 {len(filtered_results)}개 결과: {filtered_results}")
    ...
    def _calculate_streak_from_results(self, results):
        """
        결과 리스트에서 연패 정보 계산 (웹소켓 하이브리드와 동일한 로직)
        ...
        try:
            ...
            self.logger.error(f"연패 계산 오류: {e}")
