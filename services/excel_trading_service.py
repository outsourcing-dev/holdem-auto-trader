...
class ExcelTradingService:
    ...
    def process_game_results(self, game_state, game_count, current_room_name, log_on_change=False):
        ...
        if is_first_run and filtered_results:
            return self._handle_first_run(filtered_results, recent_results)
    ...
    def _handle_first_run(self, filtered_results, recent_results):
        """첫 실행 시 처리"""
        ...
        self._update_processed_rounds(filtered_results)
        
        return last_column, len(filtered_results), recent_results, next_pick
